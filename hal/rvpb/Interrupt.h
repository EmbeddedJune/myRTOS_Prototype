...
typedef union CpuControl_t {
    uint32_t all;
    struct {
        uint32_t Enable:1;          // 0, Enable CPU interface for this GIC
        uint32_t reserved:31;
    } bits;
} CpuControl_t;     // CPU interface register
...
typedef union BinaryPoint_t {
    uint32_t all;
    struct {
        // 인터럽트 우선순위 비교할 때 비트 몇 개 사용할지 결정
...
} InterruptAck_t;   // Interrupt Acknowledge register
...
typedef union RunningInterrupt_t {
    ...
    struct {
        uint32_t Reserved:4;
        uint32_t Priority:4;        // 7:4, 현재 실행되는 인터럽트 우선순위
        uint32_t reserved:24;
    } bits;
} RunningInterrupt_t;
...
typedef union DistributorCtrl_t {
    uint32_t all;
    struct {
        uint32_t Enable:1;          // 0, Enable interrupt for this GIC
        uint32_t reserved:31;
    } bits;
} DistributorCtrl_t;
...
#define GIC_CPU_BASE    0x1E000000      // CPU interface register base address
#define GIC_DIST_BASE   0x1E001000      // Distributor register base address
#define GIC_PRIORITY_MASK_NONE 0xF      // No masking
// GIC 인터럽트 ID는 32번(watchdog0)부터 95번까지 할당돼있다.
