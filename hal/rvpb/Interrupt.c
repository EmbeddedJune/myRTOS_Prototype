...
static InterHdlr_fptr sHandlers[INTERRUPT_HANDLER_MAX_NUM];
...
void Hal_interrupt_init(void)
{
    ...
    for (uint32_t i = 0; i < INTERRUPT_HANDLER_MAX_NUM; ++i) sHandlers[i] = NULL;
    enable_irq();
}

void Hal_interrupt_enable(uint32_t interrupt_num)
{
    if ((interrupt_num < GIC_IRQ_START) || (interrupt_num > GIC_IRQ_END)) return;
...
void Hal_interrupt_run_handler(void)
{
    ...
    if (sHandlers[interrupt_num] != NULL) sHandlers[interrupt_num]();
